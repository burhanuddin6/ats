...
class CandidateTestCase(TestCase):
    ...
    def test_create_candidates(self):
        ...
        for i in range(3):
            ...
            self.assertEqual(authenticate(username=self.emails[i], password=self.passwords[i]), candidates[i].user)
...
class SlotGroupTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.jobs.append(a_models.Job.objects.create(
            ...
            created_By=a_models.Recruiter.objects.create(
                ...
                user=User.objects.create_user(email="abc@gmail.com", password="password", username="username")
            ))
        )
    ...
    def test_job(self):
        ...
        self.assertEqual(job.description, 'test description', message)
...
class RecruiterSiteTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.recruiter = a_models.Recruiter.objects.create(
            first_Name='test first name',
            last_Name='test last name',
            user=User.objects.create_user(email="recruiter@ats.com", username="username", password="password")
        )
        self.recruiters_grp.user_set.add(self.recruiter.user)
    
    def test_setUp(self):
        """Checks if the setup was correct.
        This test is important for the other tests to run correctly. The following test need the recruiter to be able to login
    ...
    def test_login(self):
        ...
        self.assertEqual(response.status_code, 200, "GET request to /recruiters/login should return status code 200.")
        self.assertTrue(isinstance(response.context['login_form'], r_forms.LoginForm), "login_form should be an instance of r_forms.LoginForm.")
        self.assertTemplateUsed(response, 'recruiters/login.html', "recruiters/login.html should be used for rendering the login page.")
        # TODO : Add user authentication test
        response = c.post('/recruiters/login', {}, follow=True, content_type='application/x-www-form-urlencoded')
        self.assertEqual(response.status_code, 200, "POST request to /recruiters/login should return status code 200.")
        # login_form = r_forms.LoginForm({'username':recruiter.user.email, 'password': 'password'})

        logged_in = c.login(username=self.recruiter.user.username, password="password")
        response = c.get('/recruiters/login')
        self.assertEqual(response.status_code, 302, "GET request to /recruiters/login with user logged in should return status code 302 due to redirection to index.")


    def test_dashboard(self):
        """Checks the dashboard page of the recruiter site.
        Checks if the correct template is used
        Checks the status codes of the dashboard page on GET requests with user logged in and logged out.
        """
        c = Client()
        ...
        self.assertEqual(
            ...
            "GET request to /recruiters/dashboard with user not logged in should return status code 302."
        )
        ...
        self.assertEqual(
            ...
            "GET request to /recruiters/dashboard with user logged in should return status code 200."
        )
    ...
    def test_jobs(self):
        ...
        self.assertEqual(response.status_code, 302, "GET request to /recruiters/jobs should return status code 302.")

        # A pretest to check that the recruiter is correctly defined in setUp
        logged_in = c.login(username=self.recruiter.user.username, password="password")

        response = c.get('/recruiters/jobs')
        self.assertEqual(response.status_code, 200, "GET request to /recruiters/jobs should return status code 200.")
        self.assertTrue(isinstance(response.context['jobs'], QuerySet), "jobs should be an instance of QuerySet.")
        self.assertTemplateUsed(response, 'recruiters/jobs.html', "recruiters/jobs.html should be used for rendering the jobs page.")